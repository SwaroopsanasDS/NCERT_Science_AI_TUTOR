...
st.set_page_config(page_title="Curiosity AI – Science Tutor", page_icon="🧪", layout="wide")

with st.sidebar:
    st.markdown("## 🧠 How to Use")
    st.write("Type a science question from Class 8 syllabus and get answers with references!")
    st.markdown("---")
...
if st.button("✨ Ask AI"):
    if not query.strip(): st.warning("Please enter a question!")
        ...
        if answer: st.success(answer.strip())
        if sources:
